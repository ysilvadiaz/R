...
ggplot(card_fraud, aes(x = as.factor(is_fraud), y = amt, fill = as.factor(is_fraud))) +
  geom_boxplot(alpha = 0.6) +
  scale_y_log10() +
  labs(title = "Transaction Amount by Fraud Status", x = "Fraud", y = "Amount (log scale)", fill = "Fraud")
```
__CONCLUSION:__
...
# Boxplot: Age by Fraud Status
ggplot(card_fraud, aes(x = as.factor(is_fraud), y = age, fill = as.factor(is_fraud))) +
  geom_boxplot(alpha = 0.6) +
  labs(title = "Cardholder Age by Fraud Status", x = "Fraud", y = "Age", fill = "Fraud")


```
__CONCLUSION:__
The age variable covers a wide range of cardholder ages, with clusters in young and middle-aged groups. Scatterplots and boxplots show transaction amounts vary across age groups, and some groups exhibit higher-value outliers. The boxplot comparing age by fraud status reveals observable differences between fraud and non-fraud cases, indicating that age may help in distinguishing fraudulent behavior. Therefore, age should be retained as a model input.
...
cost <- xgb_preds %>%
  mutate(
    amt = exp(log_amt),                       # Convert log(amount) back to dollars
    is_fraud = as.character(is_fraud),        # Ensure character for logic
    pred = as.character(.pred_class)          # Ensure character for logic
  )
...
cost <- cost %>%
  mutate(
    ...
    false_negative = ifelse(is_fraud == "1" & pred == "0", amt, 0),
    # False positives: we thought they were fraud, but they were not
    false_positive = ifelse(is_fraud == "0" & pred == "1", amt, 0),
    # True positives: we thought they were fraud, and they were
    true_positive  = ifelse(is_fraud == "1" & pred == "1", amt, 0),
    # True negatives: we thought they were ok, and they were
    true_negative  = ifelse(is_fraud == "0" & pred == "0", amt, 0),
    # Naive case: if we classify everything as not fraud, we miss all fraud
    naive_cost     = ifelse(is_fraud == "1", amt, 0)
  )

# 5. Summarize total costs for the model vs naive baseline
total_costs <- cost %>%
  summarise(
    Total_Fraud_Missed_by_Model = sum(false_negative),
    Total_Fraud_Missed_by_Naive = sum(naive_cost)
  )
